...
export default function Examenes() {
  ...
  useEffect(() => {
    if (response.data) {
      ...
      if (response.data.length > 0) {
        ...
        } else if (response.data[0].idExam !== undefined && response.data[0].idExam !== null) {
          setDataExam(response.data)
          console.log('response93', response)
        } else {
  ...
  return (
    <div>
      ...
      {
        examenes && (
      <Modal onClose={handleCloseExamenes}>
        <h2>Tratamientos</h2>
        <TableContainer sx={{ minWidth: 800 }}>
          <Table>
            <UserListHead
            ...
            <TableBody>
              {dataExam.map((row) => {
                ...
                // eslint-disable-next-line no-unused-expressions
                return (
                  <TableRow hover key={idExam} tabIndex={0}>
                    <TableCell style={{ fontSize: 10, paddingLeft: '16px' }} component="th" scope="row" padding="none">
                      {idExam}
                    </TableCell>
